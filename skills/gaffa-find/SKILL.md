...
3. Targeted extraction. When the value sits in a stable element, prefer a `selector` with a direct capture, which is cheaper and repeatable. When it is buried in free text or the goal is interpretive (a salary somewhere in a job description, a page summary), use the `parse_json` action (inline `data_schema`, or `data_schema_id` for a reused shape), which is LLM-backed and handles the ambiguity a selector cannot but is token-priced and can vary between runs.
