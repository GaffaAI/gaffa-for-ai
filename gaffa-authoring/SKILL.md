...
- Proxy locations: `us`, `ie`, `sg`, `fr`. Datacenter IP otherwise.
...
If both tiers fail before any code is emitted, refuse with a clear message rather than guessing from training data. If no gaffa MCP was configured, the message also suggests registering `https://gaffa.dev/docs/~gitbook/mcp` for faster lookups. Otherwise the message is: "Live gaffa docs are currently unavailable. Please retry, or consult https://gaffa.dev/docs manually."
